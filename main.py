...
def lost_and_found():
    ...
    if request.method == 'POST':
        ...
        user = cursor.fetchone()
        user_id = user['ResidentID']
        sql = "INSERT INTO lost_items (user_id, item_name, image_data, location,Date, description) VALUES (%s, %s, %s, %s,%s, %s)"
        cursor.execute(sql, (user_id, item_name, image_data, location,date, description))
        mysql.connection.commit()
    ...
    for item in lost_items:
        ...
        cursor.execute("SELECT ContactInfo FROM resident WHERE ResidentID = %s",(item['user_id'],))
        temp=cursor.fetchone()
        item['contact'] = temp['ContactInfo']
    return render_template('lostandfound.html',lost_items=lost_items,user_id = user_id)
...
def item_delete():
    ...
    # Perform the deletion of the event with the given EventID
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("DELETE FROM lost_items WHERE id = %s", (lost_id,))
    mysql.connection.commit()
...
#     # Iterate through notifications to check seats availability
#     for notification in notifications:
#         cursor.execute("SELECT seats_available FROM notifications WHERE id = %s", (notification['id'],))
#         seats = cursor.fetchone()
#         if seats and seats['seats_available'] < 0:
#             notification = None
...
#         cursor.close()
#     else:
#         flash("Seats are filled!", "danger")
#         return redirect(url_for('notification'))

#     return render_template('transport1.html', notifications=notification_id, title="Transport") # Redirect to the transportation page
